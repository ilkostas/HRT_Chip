...
**Implementation status (this repo):** Done. Guidance/objective surrogates and scoring table: [`src/hrt_chip/guidance.py`](../src/hrt_chip/guidance.py) (pin-group HPWL + RUDY when `net_pin_dx`/`net_pin_dy` exist; legality split; `netlist_pins_missing` without pins); `DiffusionSampleRequest.guidance` + optional `GuidanceObjectiveBias` / `objective_bias` in [`src/hrt_chip/diffusion.py`](../src/hrt_chip/diffusion.py); multi-weight sweep in [`src/hrt_chip/stages/generate.py`](../src/hrt_chip/stages/generate.py); `RunConfig.guidance_preset` / `guidance_weights_sweep` in [`src/hrt_chip/config.py`](../src/hrt_chip/config.py); pipeline `scoring_table`, `surrogate_proxy_alignment`, `guidance_sweep_resolved`, strict `argmin` proxy selection in [`src/hrt_chip/pipeline.py`](../src/hrt_chip/pipeline.py); CLI `--guidance-preset`, `--guidance-weight`; tests in [`tests/test_phase3_guidance.py`](../tests/test_phase3_guidance.py), [`tests/test_netlist_surrogates.py`](../tests/test_netlist_surrogates.py), [`tests/test_rank_metrics.py`](../tests/test_rank_metrics.py).
